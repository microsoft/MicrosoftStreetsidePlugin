...
import org.openstreetmap.josm.plugins.streetside.utils.ImageUtil;
...
public final class MapObjectLayer extends Layer implements ZoomChangeListener {
  ...
  private MapObjectLayer() {
    super(I18n.tr("Display Mapillary street signs"));
    NavigatableComponent.addZoomChangeListener(this);
    StreetsideProperties.MAPOBJECT_ICON_SIZE.addListener(val -> {
      scaledIcons.clear();
      ...
      synchronized (objects) {
        for (MapObject object : objects) {
          if (!scaledIcons.containsKey(object.getValue())) {
            scaledIcons.put(
              object.getValue(),
              ImageUtil.scaleImageIcon(MapObject.getIcon(object.getValue()), StreetsideProperties.MAPOBJECT_ICON_SIZE.get())
            );
          }
        }
      }
  ...
  public String getToolTipText() {
    return I18n.tr("Displays objects detected by Streetside from their street view imagery");
  }
