...
import org.openstreetmap.josm.plugins.streetside.utils.api.JsonMapObjectDecoder;
...
public class MapObjectDownloadRunnable implements Runnable {
  ...
  public void run() {
    ...
    try {
      while (nextURL != null && result.size() < StreetsideProperties.MAX_MAPOBJECTS.get() && !layer.isDownloadRunnableScheduled()) {
    ...
    if (nextURL == null) {
      ...
      layer.setStatus(result.size() >= StreetsideProperties.MAX_MAPOBJECTS.get() ? STATUS.INCOMPLETE : STATUS.DOWNLOADING);
    }
    layer.finishDownload(nextURL == null || result.size() >= StreetsideProperties.MAX_MAPOBJECTS.get());
    try {
